...
##s=ttk.Scrollbar(c,orient=VERTICAL,command=lbox.yview)
...
##s.grid(column=5,row=0,sticky=NS)
...
menu_ryan=Menu(menubar)
menubar.add_cascade(menu=menu_file, label='File')
menubar.add_cascade(menu=menu_edit, label='Edit')
menubar.add_cascade(menu=menu_ryan, label='Ryan!!!!!!')
...
def leave(*args):
    root.destroy()
menu_file.add_command(label='Exit',command=leave)
root.mainloop()
##root = Tk()
##
...
##    destination=IntVar()
##    subroot.title('Find')
##    label=ttk.Label(subroot,text='Enter Destination:',font=('Times New Roman',9)).grid(column=0,row=0)
##    destentry=ttk.Entry(subroot,textvariable=destination).grid(column=0,row=1)
##    def gtline(*args):
##        print l.curselection()
##        l.selection_set(destination.get())
##    gtbutton=ttk.Button(subroot,text='Go To Line',command=gtline).grid(column=0,row=2)
##root.bind('<Control-f>',goto)
##root.mainloop()
