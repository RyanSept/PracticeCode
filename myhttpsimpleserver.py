...
from different directories. All you have to do is
create an instance of this Handler, add the files you'd like to
serve in the paths list and set the flag to a value greater than 1.
If you have files in a different disk partition, it'd be best to
...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def create_zip(self):
        filelist = self.paths
        try:
            if not os.path.exists("/Metafora/allfiles1.zip"):
                zipf = ZipFile("/Metafora/allfiles1.zip",'w')
            else:
                zipf = ZipFile("/Metafora/allfiles1.zip",'w')
        ...
        for f in filelist:
            try:
                if os.path.isfile(f):
                    if counter+os.path.getsize(f)<maxsize:
                        zipf.write(f,os.path.basename(f))
                    else:
                        zipf.close()
                        zipf = ZipFile("/Metafora/all_files%d.zip"%zipcount+1,'w')
                        zipcount+=1
            except Exception as e:
                print str(e)
        zipf.close()
